<?php
/**
 * Test Header functions.
 *
 * @package Archetype
 * @group header
 */

class Tests_Header extends WP_UnitTestCase {

	/**
	 * Displays the header widget regions
	 */
	function test_archetype_header_widget_region() {

		$this->markTestIncomplete( 'This test has not been implemented.' );

	}

	/**
	 * Displays the Site Branding
	 */
	function test_archetype_site_branding() {

		$this->markTestIncomplete( 'This test has not been implemented.' );

	}

	/**
	 * Displays the Primary Navigation
	 */
	function test_archetype_primary_navigation() {

		$this->markTestIncomplete( 'This test has not been implemented.' );

	}

	/**
	 * Displays the Secondary Navigation
	 */
	function test_archetype_secondary_navigation() {

		$this->markTestIncomplete( 'This test has not been implemented.' );

	}

	/**
	 * Displays the Skip links
	 */
	function test_archetype_skip_links() {

		$this->markTestIncomplete( 'This test has not been implemented.' );

	}

}
